...
const NAV = [
  { label: "Intelligence", to: "/" as const },
  { label: "Investigations", to: "/investigations" as const },
  { label: "Network Analysis", to: "/network" as const },
  { label: "Evidence Upload", to: "/evidence" as const },
  { label: "Intelligence Reports", to: "/reports" as const },
];
        ...
        <nav className="hidden md:flex gap-8 items-center h-full">
          {NAV.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="text-sm text-on-surface-variant hover:text-on-surface transition-colors pb-1 font-medium"
              activeProps={{ className: "text-sm text-primary border-b border-primary pb-1 font-medium" }}
